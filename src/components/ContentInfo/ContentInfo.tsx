...
export const ContentInfo: FC<ContentInfoProps> = ({
  category,
  rating,
  views,
  isSerializing,
    ...
    <div className={cn("flex w-358 flex-col items-start gap-26", className)}>
      ...
      <div className="gap-lg relative flex h-105 w-full shrink-0 flex-col items-start">
        ...
        <div className="gap-md relative flex w-full shrink-0 flex-col items-start">
          <p className="text-body-regular w-full tracking-tight text-black">
            {category}
          </p>
        ...
        <div className="gap-xs flex items-center">
          {tags.map((tag, index) => (
            <div
              key={index}
              className="bg-background-subtle py-xs flex items-center justify-center rounded-md px-10"
            >
              <span className="text-caption-regular text-muted whitespace-nowrap">
                {tag}
              </span>
            </div>
          ))}
